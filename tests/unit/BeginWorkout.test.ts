...
describe('BeginWorkout component', () => {
    it("has start training visible", async() => {
        let startBtn = wrapper.find('[class="start-workout"]');
    ...
    it("displays picker on click", async() => {
        ...
        let selector = wrapper.find('[class="workout-selector-container"]');
        expect(selector.exists()).toBeTruthy();
    });

    it('shows all the titles the workouts ', async() => {
        let titles = wrapper.findAll('[data-test="workoutTitle"]');
